...
| Friends | `/(tabs)/friends` | `search`, `get_user_info`, `get_list_blocks`, `set_block` | UI shell + API-ready fallback | Top navigator section preserved from leader UI. Friend-specific candidates `get_user_friends`, `get_list_friends`, and `get_friends` currently return deployed 404, so use search/user/block APIs until backend confirms a route. |
...
| Search | `/search` | `search`, `get_saved_search`, `del_saved_search` | Server-backed compact UI | Non-tab route opened from top search actions; supports profile search via `user_id`; saved search delete is present. |
| Courses / enrollment | `/courses` | `get_list_courses_of_student`, `get_list_students`, `get_requested_enrollment`, `set_request_course`, `set_approve_enrollment` | Server-backed compact UI | Non-tab route linked from Profile/notifications; student request state is pending/requested until backend approval. |
