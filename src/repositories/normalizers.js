...
export function normalizeVideo(raw = {}, index = 0) {
  ...
  return {
    ...
    angle: firstValue(
      raw.angle,
      raw.camera,
      index === 0 ? "Góc quay trái" : "Góc quay phải",
    ),
...
export function normalizePost(raw = {}, source = ACTIVE_SOURCES.SERVER) {
  const user =
    ...
    raw.account ||
    {};
  const rawVideos = extractList(
    raw.videos || raw.video || raw.video_url || raw.images || raw.media,
  );
  ...
  const canCommentValue = firstValue(
    raw.canComment,
    raw.can_comment,
    raw.can_comment_mark,
    true,
  );

  const isTeacherExerciseLikePost = role === "GV" && Boolean(courseId);
  // console.log("like count: ", raw.like);
  // console.log("comment count: ", raw.comment);
  // console.log(typeof Number(raw.like), typeof Number(raw.comment));
  return {
    ...
    isLiked: normalizeLiked(
      firstValue(raw.isLiked, raw.is_liked, raw.liked, false),
    ),
    canComment: normalizeCanComment(canCommentValue),
    canEdit: Boolean(firstValue(raw.canEdit, raw.can_edit, false)),
    canSubmit: Boolean(
      firstValue(raw.canSubmit, raw.can_submit, isTeacherExerciseLikePost),
    ),
    courseId,
    exerciseId,
