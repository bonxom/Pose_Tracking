...
export function normalizeSession(raw = {}) {
  ...
  const username = firstValue(data.username, data.name, data.fullname, data.fullName, data.phonenumber);
...
export function normalizePost(raw = {}, source = ACTIVE_SOURCES.SERVER) {
  ...
  const videos = extractList(raw.videos || raw.video || raw.video_url || raw.images).map(normalizeVideo);
  ...
  return {
    ...
    canComment: firstValue(raw.canComment, raw.can_comment, true) !== false,
